...
- `OPENAI_API_KEY` - ключ OpenAI для свободных реплик пользователя.
- `OPENAI_MODEL` - модель OpenAI. Если оставить пустой, код использует компактный вариант по умолчанию.
...
## Как работает свободный текст

Клиент не вызывает OpenAI напрямую. Любая свободная реплика отправляется в `app/api/chat/route.ts`, а серверный модуль `lib/ai.ts`:

- получает текущий шаг;
- получает уже собранные ответы;
- отвечает коротко на русском;
- возвращает пользователя к следующему вопросу сценария.

Если `OPENAI_API_KEY` не задан, используется безопасная локальная логика возврата в сценарий, чтобы форма не ломалась в локальной разработке.
...
- простой in-memory rate limit для `/api/chat` и `/api/lead`.
...
Важно: не используйте drag-and-drop загрузку статической папки. В проекте есть Next.js route handlers `/api/chat` и `/api/lead`, поэтому нужен полноценный deploy через Git/Netlify build.
...
Для свободных AI-ответов дополнительно:

- `OPENAI_API_KEY`
- `OPENAI_MODEL`
- `OPENAI_BASE_URL`, если используете не стандартный OpenAI endpoint
